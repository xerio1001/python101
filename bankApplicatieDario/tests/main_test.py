...
def test_ValidValuesForPersonConstructor(persoon1):
    assert (
        persoon1.naam == "Van Hasselt"
        and persoon1.voornaam == "Dario"
        and persoon1.rijksregisternummer == "01.10.02-149.08"
    )
...
@pytest.mark.parametrize(("value", "result"), [(500, 1000), (50, 550), (10000, 10500)])
...
@pytest.mark.parametrize(("value", "result"), [(499, 1), (1, 499), (500, 0), (0, 500)])
...
@pytest.mark.parametrize("value", [501, 1000, 1234])
...
@pytest.mark.parametrize(
    ("value", "zichtResult", "spaarResult"), [(200, 300, 1200), (500, 0, 1500)]
)
def test_zichtSufficientAmountByTransfer(
    zicht1, spaar1, value, zichtResult, spaarResult
):
...
def test_creatieSpaarRekening(zicht1):
    ...
    assert (
        spaar1.saldo == 1000
        and spaar1.bankrekeningNummer == "091-0122401-16"
        and spaar1.zicht == zicht1
    )
...
@pytest.mark.parametrize(
    ("value", "spaarResult", "zichtResult"),
    [(200, 800, 700), (500, 500, 1000), (1000, 0, 1500)],
)
def test_spaarSufficientAmountByTransfer(
    spaar1, zicht1, value, zichtResult, spaarResult
):
